import type { PropertyDetailsValues } from "@/components/intelligence/PropertyDetailsForm";
import { compoundFutureValue, type LocationBenchmark } from "@/lib/locationBenchmarks";

export function parsePurchasePrice(raw: string): number {
  return parseFloat(String(raw || "0").replace(/,/g, "")) || 0;
}

/** Maps form bedroom option to multiplier input. */
...
/** Representative years for horizon bands (short 1–3, medium 3–5, long 5+). */
...
export function hasIntelPreviewInputs(values: PropertyDetailsValues): boolean {
  return Boolean(
