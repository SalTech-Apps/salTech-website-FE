...
export default function PageBySlug() {
	...
	useEffect(() => {
		...
		async function load() {
			...
			try {
				...
				if (cancelled) return;
				const data = pages.find((page) => page.slug === slug);
				if (data) {
	...
	return (
		<div
			className={`min-h-screen bg-main-background pb-12 sm:pb-20 ${!bannerImageUrl ? "pt-20 sm:pt-24 lg:pt-32" : ""}`}
		>
			<JsonLd
				data={webPageSchema({
					name: seoTitle || title,
					description:
						seoDescription ||
						`${title} — SalTech digital products and engineering`,
					path: `/page/${slug}`,
				})}
			/>
			{bannerImageUrl && (
