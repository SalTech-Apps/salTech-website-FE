...
interface SendNewsletterEmailRequest {
  to: string | string[];
  subject: string;
  text?: string;
  html?: string;
 ...
 */
export const sendNewsletterEmail = onCall(
  ...
  async (request) => {
    ...
    if (!email || !ADMIN_EMAILS.includes(email)) {
      throw new HttpsError(
        "permission-denied",
        "Only admins can send newsletter emails"
      );
    }

    const { to, subject, text, html } = request.data as SendNewsletterEmailRequest;

    if (!to || (Array.isArray(to) && to.length === 0)) {
      throw new HttpsError("invalid-argument", "Missing or empty recipient(s)");
    }
    if (!subject || typeof subject !== "string" || !subject.trim()) {
      throw new HttpsError("invalid-argument", "Missing or empty subject");
    }
    ...
    if (!user || !pass) {
      throw new HttpsError(
        "failed-precondition",
        "SMTP credentials not configured. Set SMTP_USER and SMTP_PASS secrets."
      );
    }
...
interface ContactFormNotificationRequest {
  fullName: string;
  email: string;
  phone: string;
  subject: string;
...
interface SurveySubmissionEmailRequest {
  email: string;
}
 ...
 */
export const sendContactFormNotification = onCall(
  { secrets: [smtpUser, smtpPass, smtpHost, smtpPort] },
  async (request) => {
    const data = request.data as ContactFormNotificationRequest;
    if (!data?.fullName || !data?.email || !data?.message) {
      throw new HttpsError("invalid-argument", "Missing required contact form fields");
    }
 ...
 */
export const sendSurveySubmissionEmail = onCall(
  { secrets: [smtpUser, smtpPass, smtpHost, smtpPort] },
  async (request) => {
    const data = request.data as SurveySubmissionEmailRequest;
    ...
    try {
      ...
      throw new HttpsError(
        "internal",
        err instanceof Error ? err.message : "Failed to send survey email"
      );
    }
  }
);
...
interface IntelligenceReportRequest {
  email: string;
  propertyType: string;
  location: string;
  bedrooms: string;
 ...
 */
export const sendIntelligenceReport = onCall(
  { secrets: [smtpUser, smtpPass, smtpHost, smtpPort] },
  async (request) => {
    const data = request.data as IntelligenceReportRequest;
    if (!data?.email || typeof data.email !== "string" || !data.email.trim()) {
      throw new HttpsError("invalid-argument", "Valid email is required");
    }
...
function buildIntelligenceReportHtml(data: IntelligenceReportRequest): string {
  ...
  const priceNum = parsePurchasePrice(data.purchasePrice || "0");
  const benchmark = getLocationBenchmarkFromFormSlug(data.location || "");
  const bedMult = getBedroomMultiplier(
    parseBedroomsForIntel(data.bedrooms || "2")
  );
  ...
  const rental = buildRentalMetrics(benchmark, priceNum, bedMult);
  const horizonYears = getHorizonYears(data.investmentHorizon || "");
  const growthRates = getAdjustedGrowthRates(benchmark, data.offPlan || "");
  const horizonFv = buildCapitalHorizonValues(
    priceNum,
    growthRates,
    horizonYears
  );
  ...
  function benchmarkNoteHtml(forRent: boolean): string {
  ...
  const dataDisclaimer = `Market Data Last Updated: ${MARKET_DATA_DISCLAIMER.lastUpdatedYear}. Sources: ${MARKET_DATA_DISCLAIMER.sourcesLine}.`;
